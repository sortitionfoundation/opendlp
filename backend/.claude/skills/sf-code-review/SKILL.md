...
- Has anything been added to `config.py` (or removed)? Any new feature flags, or have we cleaned up any? If so, are there examples in `env.example` and explanations in `docs/configuration.md`. A new setting also has to be classified in `tests/conftest.py`: `LEAKY_ENV_KEYS`/`LEAKY_ENV_PREFIXES` if a value in someone's `.env` could change how a test behaves, `ENV_KEYS_TESTS_MAY_INHERIT` if the suite needs it from `.env`. `tests/unit/test_env_scrub.py` fails until it is, so check the classification is the right one rather than whichever one made the test pass - putting a behaviour-changing setting on the inherit list is how the leak comes back
...
- If templates draw an icon: it must be a macro imported from `backoffice/components/icons.html`, never an inline `<svg>` (`tests/unit/test_icons.py` enforces this; flag any change that weakens it or pads its allowlist). New icons come from the Lucide set on the "Icons" sheet of the OpenDLP - UI Figma file — flag a glyph hand-drawn or taken from another family (Heroicons, Material), a per-call-site stroke-width, or a glyph missing `aria-hidden="true"`. See "Icons" in `docs/agent/frontend_design_system.md`.
...
- Are new user-facing strings wrapped for translation at all, and wrapped correctly? `_()` for immediate translation, `_l()` for anything evaluated at import time - a class attribute, a default argument, a module-level constant - where `_()` would freeze the first request's language for the life of the process. Two constructs pass review by eye and are wrong: a bare literal sitting inside a `_()` call beside a lookup (`_(ERROR_MESSAGES["parse_error"])`) puts the dict **key** into the catalogue, because Babel's extractor collects every literal between the parens - hoist the lookup to a constant, as `src/opendlp/service_layer/error_translation.py` does; and a lone `%` inside a msgid fails at render time with "incomplete format", because Flask-Babel's newstyle gettext runs printf formatting over the result - concatenate it outside the call. Placeholders are worth reading character by character: `%(url) s` interpolates correctly but is a trap for translators, who have to reproduce the oddity verbatim. `just translate-check` catches the dict-key artefact and broken placeholders; it cannot catch an unwrapped string or the wrong one of `_()`/`_l()`
- Was an existing msgid **reworded**? Changing the English discards the translation for that string in every language - the entry becomes a new msgid with an empty msgstr and the app silently reverts to English there. Nothing in CI reports this, and the diff shows only the English. Flag any reword of a string that already has translations and ask whether it is worth the retranslation; a genuine wording fix usually is, an incidental tidy-up is not
...
- Generated files - `uv.lock` `package-lock.json` `migrations/versions/*.py` and the catalogues themselves (`translations/**/*.po` `translations/**/*.pot` `translations/**/*.mo`). Note this exempts the generated catalogues only: a change to a `translate-*` recipe, to `babel.cfg`, or to a translation style guide under `translations/` is hand-written and in scope
