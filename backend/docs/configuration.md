...
| `FF_OPEN_SIGNUP`       | `false`                             | Lets people register at `/auth/register` without an invite code: a no-invite signup gets the `user` global role, and the form asks the optional signup survey questions (an invite link can append `?skipq=1` to hide them). When unset or `false`, registration is invite-only and unchanged.                                        |
