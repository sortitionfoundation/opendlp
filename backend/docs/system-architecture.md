...
| Microsoft OAuth 2.0 | `app` (and user browser) | Sign-in with Microsoft. Secret expiry is surfaced by the health check. Microsoft also fetches `/.well-known/microsoft-identity-association.json` from `app` to verify the app's publisher domain. | `OAUTH_MICROSOFT_CLIENT_ID`, `OAUTH_MICROSOFT_CLIENT_SECRET`, `OAUTH_MICROSOFT_CLIENT_SECRET_EXPIRY`, `OAUTH_MICROSOFT_APPLICATION_ID`. See [microsoft_oauth_setup.md](microsoft_oauth_setup.md). |
