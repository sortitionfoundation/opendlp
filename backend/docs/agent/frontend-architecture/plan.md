...
- Put a default `user_msg()` on `OpenDLPError` in `src/opendlp/service_layer/exceptions.py`. The safest default is _not_ to return `str(self)` — that would silently make every exception in the tree claim to be user-safe, which is the current problem with extra steps. Two candidate defaults, to settle when implementing: raise `NotImplementedError` (forces an explicit opt-in per class, but turns a handled error into a crash), or return a generic translated `_("Something went wrong")` (safe, but a class that forgets to override degrades quietly to a useless message). Leaning towards the generic-string default plus a `sf-code-review` check (§7) that new user-facing exceptions override it.
...
### `dev_msg()` — dev-only errors that tell you where to look
...
Plan: add a `dev_msg()` alongside `user_msg()` — same message, with a "check the Flask console log for the full error" suffix appended:

```python
def dev_msg(self) -> str:
    # Dev-only surfaces: the full traceback is in the Flask console, but the
    # developer is looking at the page, so point them at it explicitly.
    return f"{self.user_msg()} {_('(check the Flask console log for full error details)')}"
```

And in `dev.py`, error responses use `dev_msg()`. For the truly-unexpected case (the outer catch-all in `service_docs_execute`), the response is a generic string with the same suffix — the dev gets "something blew up, look at the terminal", which is honest and actionable, rather than either a leaked traceback string or a dead end.

Open sub-question for whoever implements: whether `dev_msg()` lives on the exception base class or is a helper function in `dev.py` (e.g. `_dev_error(exc)`). Putting it on the base class means production code has a method it must never call — a footgun in the opposite direction. A `dev.py`-local helper keeps the dev-only concern in the dev-only file, which I mildly prefer; flagging it rather than deciding, since it's a five-minute change either way once the direction is agreed.
...
4. **Phase 1c — error-handling convention.** Add `user_msg()` to the exception hierarchy and switch the two `str(e)` call sites in `backoffice_registration.py` (own commit). Then narrow the five `dev.py` handlers and add the `dev_msg()` console-log hint (§3). Write the convention up in the docs (§6) and add the checks to `sf-code-review` (§7).
...
- **Minor, for whoever implements §3:** does `dev_msg()` live on the exception base class or as a `dev.py`-local helper? (Leaning helper — keeps a dev-only concern out of the shared hierarchy.) And what should the `user_msg()` default on `OpenDLPError` be: `NotImplementedError` or a generic translated string? (Leaning generic string.)
