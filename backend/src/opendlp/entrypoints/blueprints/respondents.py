...
def _run_csv_import(
    ...
    if errors:
        ...
        # The alert renders newlines as line breaks (white-space: pre-line) and
        # auto-escapes the CSV-supplied values, so this stays a plain string:
        # flask-session serialises flashes with msgspec, which rejects Markup.
        lines = [summary, *errors[:_MAX_FLASH_ERROR_LINES]]
...
def upload_respondents_csv(assembly_id: uuid.UUID) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.warning("Invalid CSV format for respondents upload", assembly_id=str(assembly_id), error=str(e))
        ...
        logger.warning(
            "Insufficient permissions to upload respondents for assembly",
            assembly_id=str(assembly_id),
        ...
        logger.warning("Assembly not found for respondents upload", assembly_id=str(assembly_id), error=str(e))
...
def delete_respondents(assembly_id: uuid.UUID) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.warning(
            "Insufficient permissions to delete respondents for assembly",
            assembly_id=str(assembly_id),
            user_id=str(current_user.id),
            error=str(e),
        )
        ...
        logger.warning("Assembly not found for respondents deletion", assembly_id=str(assembly_id), error=str(e))
        flash(_("Assembly not found"), "error")
        return redirect(url_for("backoffice.dashboard"))
    except Exception as e:
        logger.exception(
...
def delete_respondent_route(assembly_id: uuid.UUID, respondent_id: uuid.UUID) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.warning(
            "Insufficient permissions to delete respondent",
        ...
        logger.warning(
            "Respondent not found in assembly",
            respondent_id=str(respondent_id),
            assembly_id=str(assembly_id),
            error=str(e),
        )
        ...
        logger.warning("Assembly not found for respondent deletion", assembly_id=str(assembly_id), error=str(e))
        flash(_("Assembly not found"), "error")
        return redirect(url_for("backoffice.dashboard"))
    except Exception as e:
        logger.exception(
