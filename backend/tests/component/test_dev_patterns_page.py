...
class TestPatternsPageScripts:
    def test_loads_the_patterns_bundle(self, patterns_html):
    ...
    def test_the_bundle_is_cache_busted_and_nonced(self, patterns_html):
    ...
    def test_registers_its_components_before_alpine_starts(self, patterns_html):
        ...
        """
        alpine_tag = re.search(r"<script[^>]*alpine-csp\.js[^>]*>", patterns_html, re.DOTALL)
        bundle_tag = re.search(r"<script[^>]*backoffice/js/patterns\.js[^>]*>", patterns_html, re.DOTALL)
    ...
    def test_carries_no_inline_script_body_at_all(self, patterns_html):
        ...
        """
        inline_bodies = re.findall(r"<script(?![^>]*\ssrc=)[^>]*>(.*?)</script>", patterns_html, re.DOTALL)
