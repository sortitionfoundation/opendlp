# ABOUTME: Component tests for the legacy db_selection blueprint over a FakeUnitOfWork
# ABOUTME: Drives auth/validation/render/redirect, readiness + non-pool warnings, reset, downloads (sortition boundary-stubbed)
...
def _mapped_domain_objects():
    """Settings POST writes SelectionSettings via SQLAlchemy flag_modified, which needs mapped classes."""
    database.start_mappers()


@pytest.fixture
def assembly_for_db_selection(fake_store, admin_user):
...
class TestDbSelectionRoutes:
    ...
    def test_view_db_selection_with_run_shows_status(self, logged_in_admin, assembly_for_db_selection, fake_store):
        assembly = assembly_for_db_selection
        task_id = uuid.uuid4()
    ...
    def test_progress_polling_returns_fragment(self, logged_in_admin, assembly_for_db_selection, fake_store):
        assembly = assembly_for_db_selection
        task_id = uuid.uuid4()
    ...
    def test_progress_polling_completed_sends_refresh(self, logged_in_admin, assembly_for_db_selection, fake_store):
        assembly = assembly_for_db_selection
        task_id = uuid.uuid4()
    ...
    def test_save_db_selection_settings(self, logged_in_admin, assembly_for_db_selection, fake_store):
        assembly = assembly_for_db_selection
    ...
    def test_save_db_selection_settings_marks_confirmed(self, logged_in_admin, admin_user, fake_store):
...
class TestDbSelectionErrorHandling:
    ...
    def test_progress_wrong_assembly(self, logged_in_admin, assembly_for_db_selection, fake_store, admin_user):
        assembly = assembly_for_db_selection
        task_id = uuid.uuid4()
    ...
    def test_start_db_selection_redirects_to_settings_when_not_confirmed(self, logged_in_admin, admin_user, fake_store):
    ...
    def test_check_db_data_redirects_to_settings_when_not_confirmed(self, logged_in_admin, admin_user, fake_store):
    ...
    def test_view_db_selection_with_run_wrong_assembly(
        self, logged_in_admin, assembly_for_db_selection, fake_store, admin_user
    ):
        assembly = assembly_for_db_selection
        task_id = uuid.uuid4()
...
class TestSelectionReadinessWarnings:
    ...
    def test_shows_no_settings_warning(self, logged_in_admin, admin_user, fake_store):
    ...
    def test_no_warnings_when_all_ready(self, logged_in_admin, admin_user, assembly_for_db_selection, fake_store):
...
class TestNonPoolRespondentWarning:
    ...
    def _import_and_select_respondents(self, fake_store, admin_user, assembly_id):
    ...
    def test_selection_page_shows_warning_with_non_pool_respondents(
        ...
        assembly = assembly_for_db_selection
        self._import_and_select_respondents(fake_store, admin_user, assembly.id)
