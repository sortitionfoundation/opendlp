...
_downloaded_csv: dict[str, str] = {}


@given(parsers.parse('there is an assembly with respondents ready to export called "{title}"'))
def assembly_ready_to_export(title: str, assembly_creator, admin_user, test_database) -> None:
...
def open_respondents_page(admin_logged_in_page: Page, title: str) -> None:
    assembly_id = _assembly_ids[title]
    admin_logged_in_page.goto(f"{Urls.base}/backoffice/assembly/{assembly_id}/respondents?source=csv")
...
def run_export_download(admin_logged_in_page: Page) -> None:
    ...
    path = download.path()
    _downloaded_csv["content"] = pathlib.Path(path).read_text(encoding="utf-8")


@then("the downloaded CSV contains the respondent ids")
def csv_contains_ids(admin_logged_in_page: Page) -> None:
    content = _downloaded_csv["content"]
    assert "R001" in content
    assert "R002" in content
    assert "selection_status" in content
