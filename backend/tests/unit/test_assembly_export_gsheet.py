# ABOUTME: Unit tests for the AssemblyExportGSheet domain object
...
class TestPostInit:
    ...
    def test_whitespace_is_stripped_from_url(self) -> None:
        config = AssemblyExportGSheet(
            assembly_id=uuid.uuid4(), export_kind=GSheetExportKind.RESPONDENTS, url=f"  {VALID_URL}  "
        )
        assert config.url == VALID_URL
    ...
    def test_invalid_url_raises_value_error(self) -> None:
        ...
        with pytest.raises(ValueError):
            AssemblyExportGSheet(
                assembly_id=uuid.uuid4(), export_kind=GSheetExportKind.RESPONDENTS, url="not-a-google-sheet"
            )
...
class TestUpdateValues:
    ...
    def test_bumps_updated_at(self) -> None:
        ...
        config = AssemblyExportGSheet(
            assembly_id=uuid.uuid4(), export_kind=GSheetExportKind.RESPONDENTS, created_at=past, updated_at=past
        )
    ...
    def test_rejects_export_kind_update(self) -> None:
        """A row is for one kind of export; changing it would repoint the other kind's sheet."""
        config = AssemblyExportGSheet(assembly_id=uuid.uuid4(), export_kind=GSheetExportKind.RESPONDENTS, url=VALID_URL)
        with pytest.raises(ValueError):
            config.update_values(export_kind=GSheetExportKind.DASHBOARD)


class TestTheExportKind:
    def test_the_respondent_export_gets_its_own_default_worksheet(self) -> None:
        config = AssemblyExportGSheet(assembly_id=uuid.uuid4(), export_kind=GSheetExportKind.RESPONDENTS)
        assert config.export_kind == GSheetExportKind.RESPONDENTS
        assert config.worksheet_name == "Respondents"

    def test_the_kind_must_be_given(self) -> None:
        """No default: a new export kind must not be able to write over another's row."""
        with pytest.raises(TypeError):
            AssemblyExportGSheet(assembly_id=uuid.uuid4())  # type: ignore[call-arg]

    def test_the_dashboard_export_gets_its_own_default_worksheet(self) -> None:
        config = AssemblyExportGSheet(assembly_id=uuid.uuid4(), export_kind=GSheetExportKind.DASHBOARD)
        assert config.worksheet_name == "Results"

    def test_an_explicit_worksheet_name_wins(self) -> None:
        config = AssemblyExportGSheet(
            assembly_id=uuid.uuid4(),
            export_kind=GSheetExportKind.DASHBOARD,
            worksheet_name="Q3 numbers",
        )
        assert config.worksheet_name == "Q3 numbers"

    def test_the_kind_survives_a_detached_copy(self) -> None:
        config = AssemblyExportGSheet(assembly_id=uuid.uuid4(), export_kind=GSheetExportKind.DASHBOARD)
        assert config.create_detached_copy().export_kind == GSheetExportKind.DASHBOARD
