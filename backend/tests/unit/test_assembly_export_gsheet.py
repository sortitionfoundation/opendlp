...
class TestPostInit:
    def test_valid_url_is_accepted(self) -> None:
        config = AssemblyExportGSheet(assembly_id=uuid.uuid4(), url=VALID_URL)
    ...
    def test_empty_url_is_allowed(self) -> None:
        ...
        config = AssemblyExportGSheet(assembly_id=uuid.uuid4())
        assert config.url == ""

    def test_whitespace_is_stripped_from_url(self) -> None:
        config = AssemblyExportGSheet(assembly_id=uuid.uuid4(), url=f"  {VALID_URL}  ")
        assert config.url == VALID_URL
    ...
    def test_invalid_url_raises_value_error(self) -> None:
        ...
        with pytest.raises(ValueError):
            AssemblyExportGSheet(assembly_id=uuid.uuid4(), url="not-a-google-sheet")
...
class TestUpdateValues:
    ...
    def test_bumps_updated_at(self) -> None:
        ...
        config = AssemblyExportGSheet(assembly_id=uuid.uuid4(), created_at=past, updated_at=past)
...
class TestTheExportKind:
    def test_defaults_to_the_respondent_export(self) -> None:
        config = AssemblyExportGSheet(assembly_id=uuid.uuid4())
