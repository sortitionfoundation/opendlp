"""Unit tests for password reset domain model."""
...
from opendlp.domain.password_reset import PasswordResetToken, generate_reset_token
...
class TestGenerateResetToken:
    ...
    def test_custom_length(self):
        ...
        token = generate_reset_token(length=16)
...
class TestPasswordResetToken:
    """Tests for PasswordResetToken domain model."""
    ...
    def test_default_expiry_is_one_hour(self):
        """Default expiry should be 1 hour."""
        user_id = uuid.uuid4()
    ...
    def test_custom_expiry(self):
        ...
        user_id = uuid.uuid4()
        token = PasswordResetToken(user_id=user_id, expires_in_hours=2)

        expected_expiry = token.created_at + timedelta(hours=2)
    ...
    def test_invalid_expiry_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Expiry hours must be positive"):
            PasswordResetToken(user_id=user_id, expires_in_hours=0)

        with pytest.raises(ValueError, match="Expiry hours must be positive"):
            PasswordResetToken(user_id=user_id, expires_in_hours=-1)
    ...
    def test_time_until_expiry(self):
        ...
        user_id = uuid.uuid4()
        token = PasswordResetToken(user_id=user_id, expires_in_hours=1)

        time_left = token.time_until_expiry()

        # Should be approximately 1 hour
        assert timedelta(minutes=59) < time_left < timedelta(minutes=61)
    ...
    def test_equality(self):
        ...
        token1 = PasswordResetToken(user_id=user_id, token_id=token_id, token="abc123")
        token2 = PasswordResetToken(user_id=user_id, token_id=token_id, token="xyz789")
    ...
    def test_inequality(self):
        ...
        token1 = PasswordResetToken(user_id=user_id)
        token2 = PasswordResetToken(user_id=user_id)
    ...
    def test_create_detached_copy(self):
        ...
        user_id = uuid.uuid4()
        original = PasswordResetToken(user_id=user_id)
        original.use()
    ...
    def test_custom_token_string(self):
        ...
        user_id = uuid.uuid4()
        custom_token = "my-custom-token-string"

        token = PasswordResetToken(user_id=user_id, token=custom_token)
